...
print('carregando base Splink...')
spLink.data.source <- fread("E:/sourceDada/splink/speciesLink_all_10261_20171004142646.txt")

datasplink<-function(sp_search)
{
  ...
  for (s in 1:NROW(sp_search))
  {
   ...
   # sp.name <- as.character(sp_search[s])
    ...
    # busca registros 
    spLink.data.source %>% filter(spLink.data.source$scientificname %in% sp.name) -> data
